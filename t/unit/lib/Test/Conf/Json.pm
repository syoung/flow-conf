...
class Test::Conf::Json extends Conf::Json with Test::Common {
...
method testWrite ($object, $inputfile, $outputfile) {
	...
	my $input = $self->util()->getFileContents($inputfile);
	my $expected = $self->jsonparser()->decode($input);
	my $output = $self->util()->getFileContents($outputfile);
	my $actual = $self->jsonparser()->decode($output);

	is_deeply($actual, $expected, "input and output files are identical");
}
